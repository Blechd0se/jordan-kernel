...
u8 omap_ehci_ulpi_read(const struct usb_hcd *hcd, u8 reg)
{
	...
	ehci_omap_writel(hcd->regs, EHCI_INSNREG05_ULPI, reg_internal);

	/* Wait for ULPI access completion */
	while ((ehci_omap_readl(hcd->regs, EHCI_INSNREG05_ULPI)
			& (1 << EHCI_INSNREG05_ULPI_CONTROL_SHIFT))) {
		udelay(1);
		if (count-- == 0) {
	...
	val = ehci_omap_readl(hcd->regs, EHCI_INSNREG05_ULPI) & 0xFF;
	return val;
}
...
again:
	...
	while ((ehci_omap_readl(hcd->regs, EHCI_INSNREG05_ULPI)
		...
		if (count-- == 0) {
			if (retry_times--) {
				ehci_omap_writel(hcd->regs, EHCI_INSNREG05_ULPI, 0);
				goto again;
			} else {
...
static int ehci_hcd_omap_probe(struct platform_device *pdev)
{
	...
	omap->ehci->regs = hcd->regs
		+ HC_LENGTH(readl(&omap->ehci->caps->hc_capbase), 0);	
