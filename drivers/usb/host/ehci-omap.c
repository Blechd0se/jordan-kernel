...
static int omap_usb_port_ulpi_bypass(enum usbhs_omap_port_mode mode)
{
	return mode != EHCI_HCD_OMAP_MODE_ULPI_PHY;
}

static int omap_usb_port_utmi_chanel_config(enum usbhs_omap_port_mode mode)
{
	return (mode == OMAP_OHCI_PORT_MODE_PHY_6PIN_DATSE0) ||
		(mode == OMAP_OHCI_PORT_MODE_PHY_6PIN_DPDM) ||
		(mode == OMAP_OHCI_PORT_MODE_PHY_3PIN_DATSE0) ||
		(mode == OMAP_OHCI_PORT_MODE_PHY_4PIN_DPDM) ||
		(mode == OMAP_OHCI_PORT_MODE_TLL_6PIN_DATSE0) ||
		(mode == OMAP_OHCI_PORT_MODE_TLL_6PIN_DPDM) ||
		(mode == OMAP_OHCI_PORT_MODE_TLL_3PIN_DATSE0) ||
		(mode == OMAP_OHCI_PORT_MODE_TLL_4PIN_DPDM) ||
		(mode == OMAP_OHCI_PORT_MODE_TLL_2PIN_DATSE0 ||
		(mode == OMAP_OHCI_PORT_MODE_TLL_2PIN_DPDM));
}

static unsigned omap_usb_port_fslsmode(enum usbhs_omap_port_mode mode)
{
	switch (mode) {
		/* non serial modes */
...
#if defined(CONFIG_MACH_OMAP_MAPPHONE_DEFY)
...
static int ehci_hcd_omap_probe(struct platform_device *pdev)
{
	...
	memcpy(&omap->port_data, &pdata->port_mode, sizeof(omap->port_data));
	...
	omap->ehci->regs = hcd->regs
		+ HC_LENGTH(ehci, readl(&omap->ehci->caps->hc_capbase));
