...
#define QTM_OBP_TABLET_FAMILY_ID		0xA0

#define QTM_OBP_SLEEP_WAIT_FOR_BOOT		100
#define QTM_OBP_SLEEP_WAIT_FOR_RESET		500
#define QTM_OBP_SLEEP_WAIT_FOR_RESET_TABLET	8000
...
#define QTM_OBP_SLEEP_WAIT_FOR_HW_RESET_TABLET	150
...
enum {
	...
	QTM_OBJ_PROCG_NOISE_SUPPRESSION3	= 54,
	/* Max number of objects currently defined */
	QTM_OBP_MAX_OBJECT_NUM = QTM_OBJ_PROCG_NOISE_SUPPRESSION3 + 1,
};
...
struct qtm_proci_stylus_cfg {
	...
	uint8_t			sync_per_x;
	uint8_t			activecfg;
	uint8_t			activelinethr;
	uint8_t			activepwr;
	uint8_t			activeratio;
	uint8_t			bend_x;
	uint8_t			bend_y;
	uint8_t			small_target_calc;
} __attribute__ ((packed));
...
/* QTM_OBJ_PROCG_NOISE_SUPPRESSION3 T54 */
struct qtm_procg_noise_suppression3_cfg {
	uint8_t			ctrl;
	uint16_t		cfg;
	uint16_t		cal_cfg;
	uint8_t			base_freq;
	...
	uint8_t			mf_freq_2nd;
	uint8_t			mf_freq_3rd;
	uint8_t			sel_freq_max;
	uint8_t			adc_per_x_max;
	uint8_t			nl_gain;
	uint8_t			nl_thr;
} __attribute__ ((packed));
...
struct qtouch_ts_platform_data {
	...
	struct qtm_procg_noise_suppression3_cfg	noise_suppression3_cfg;
